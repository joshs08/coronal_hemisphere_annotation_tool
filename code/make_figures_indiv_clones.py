...
if __name__ == "__main__":
    ...
    for ii in range(total_slices):
        ...
        for _, row in df[df["slice_number"] == slice_number].iterrows():
            ...
            if row["barcode"] in args.highlight:
                ...
                size = 6
            ax.plot(*xyz, linestyle="", marker='o', markersize=size, color=row["subclass_color"])
        ...
        for barcode in np.unique(df["barcode"]):
            ...
            if len(clone) > 1:
                ...
                for zz, xx, yy in zip(Z, X, Y):
                    ax.plot([zt, zz], [xt, xx], [-yt, -yy], linewidth=1.5, color="black")
    ...
    #plt.show()
    fig.savefig(output_directory / "reconstruction_in_3d_highlight.pdf")
    fig.savefig(output_directory / "reconstruction_in_3d_highlight.svg")
